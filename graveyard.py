...
def render(user,data,minimum=30,limit=8):
    ...
    for r in data:
        ...
        if days>=minimum: dead.append((days,r['name'],r['html_url']))
    ...
    for i,(days,name,url) in enumerate(dead):
        x=120+(i%4)*210; y=130+(i//4)*165
        out.append(f'<a href="{esc(url)}"><path class="stone" d="M{x-42} {y+15}V{y-42}C{x-42} {y-64} {x+42} {y-64} {x+42} {y-42}V{y+15}M{x-52} {y+15}H{x+52}"/><text class="name" x="{x}" y="{y-18}" text-anchor="middle">{esc(name[:20])}</text><text class="age" x="{x}" y="{y+38}" text-anchor="middle">{days} days quiet</text>{plants(days,x,y+15)}</a>')
...
def main():
    ...
    Path(a.output).write_text(render(a.user,fetch(a.user,os.getenv('GITHUB_TOKEN')),a.minimum_days,a.limit))
